#ifndef _ROS_rtabmap_InfoEx_h
#define _ROS_rtabmap_InfoEx_h
...
namespace rtabmap
{
  ...
  class InfoEx : public ros::Msg
  {
    public:
      ...
      float * statsValues;
      uint8_t refWordsKeys_length;
      int32_t st_refWordsKeys;
      int32_t * refWordsKeys;
      uint8_t refWordsValues_length;
      rtabmap::KeyPoint st_refWordsValues;
      rtabmap::KeyPoint * refWordsValues;
      uint8_t loopWordsKeys_length;
      int32_t st_loopWordsKeys;
      int32_t * loopWordsKeys;
      uint8_t loopWordsValues_length;
      rtabmap::KeyPoint st_loopWordsValues;
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (u_localLoopClosureId.base >> (8 * 3)) & 0xFF;
      ...
      offset += this->mapCorrection.serialize(outbuffer + offset);
      offset += this->loopClosureTransform.serialize(outbuffer + offset);
      offset += this->currentPose.serialize(outbuffer + offset);
      ...
      *(outbuffer + offset++) = refWordsKeys_length;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      for( uint8_t i = 0; i < refWordsKeys_length; i++){
      union {
        int32_t real;
        uint32_t base;
      } u_refWordsKeysi;
      u_refWordsKeysi.real = this->refWordsKeys[i];
      *(outbuffer + offset + 0) = (u_refWordsKeysi.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_refWordsKeysi.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_refWordsKeysi.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_refWordsKeysi.base >> (8 * 3)) & 0xFF;
      offset += sizeof(this->refWordsKeys[i]);
      }
      *(outbuffer + offset++) = refWordsValues_length;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      for( uint8_t i = 0; i < refWordsValues_length; i++){
      offset += this->refWordsValues[i].serialize(outbuffer + offset);
      }
      *(outbuffer + offset++) = loopWordsKeys_length;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      for( uint8_t i = 0; i < loopWordsKeys_length; i++){
      union {
        int32_t real;
        uint32_t base;
      } u_loopWordsKeysi;
      u_loopWordsKeysi.real = this->loopWordsKeys[i];
      *(outbuffer + offset + 0) = (u_loopWordsKeysi.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_loopWordsKeysi.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_loopWordsKeysi.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_loopWordsKeysi.base >> (8 * 3)) & 0xFF;
      offset += sizeof(this->loopWordsKeys[i]);
      }
      *(outbuffer + offset++) = loopWordsValues_length;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      *(outbuffer + offset++) = 0;
      for( uint8_t i = 0; i < loopWordsValues_length; i++){
      offset += this->loopWordsValues[i].serialize(outbuffer + offset);
      }
      return offset;
    }
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += sizeof(this->localLoopClosureId);
      offset += this->mapCorrection.deserialize(inbuffer + offset);
      offset += this->loopClosureTransform.deserialize(inbuffer + offset);
      offset += this->currentPose.deserialize(inbuffer + offset);
      ...
      uint8_t refWordsKeys_lengthT = *(inbuffer + offset++);
      if(refWordsKeys_lengthT > refWordsKeys_length)
        this->refWordsKeys = (int32_t*)realloc(this->refWordsKeys, refWordsKeys_lengthT * sizeof(int32_t));
      offset += 3;
      refWordsKeys_length = refWordsKeys_lengthT;
      for( uint8_t i = 0; i < refWordsKeys_length; i++){
      union {
        int32_t real;
        uint32_t base;
      } u_st_refWordsKeys;
      u_st_refWordsKeys.base = 0;
      u_st_refWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_st_refWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_st_refWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_st_refWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->st_refWordsKeys = u_st_refWordsKeys.real;
      offset += sizeof(this->st_refWordsKeys);
        memcpy( &(this->refWordsKeys[i]), &(this->st_refWordsKeys), sizeof(int32_t));
      }
      uint8_t refWordsValues_lengthT = *(inbuffer + offset++);
      if(refWordsValues_lengthT > refWordsValues_length)
        this->refWordsValues = (rtabmap::KeyPoint*)realloc(this->refWordsValues, refWordsValues_lengthT * sizeof(rtabmap::KeyPoint));
      offset += 3;
      refWordsValues_length = refWordsValues_lengthT;
      for( uint8_t i = 0; i < refWordsValues_length; i++){
      offset += this->st_refWordsValues.deserialize(inbuffer + offset);
        memcpy( &(this->refWordsValues[i]), &(this->st_refWordsValues), sizeof(rtabmap::KeyPoint));
      }
      uint8_t loopWordsKeys_lengthT = *(inbuffer + offset++);
      if(loopWordsKeys_lengthT > loopWordsKeys_length)
        this->loopWordsKeys = (int32_t*)realloc(this->loopWordsKeys, loopWordsKeys_lengthT * sizeof(int32_t));
      offset += 3;
      loopWordsKeys_length = loopWordsKeys_lengthT;
      for( uint8_t i = 0; i < loopWordsKeys_length; i++){
      union {
        int32_t real;
        uint32_t base;
      } u_st_loopWordsKeys;
      u_st_loopWordsKeys.base = 0;
      u_st_loopWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_st_loopWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_st_loopWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_st_loopWordsKeys.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->st_loopWordsKeys = u_st_loopWordsKeys.real;
      offset += sizeof(this->st_loopWordsKeys);
        memcpy( &(this->loopWordsKeys[i]), &(this->st_loopWordsKeys), sizeof(int32_t));
      }
      uint8_t loopWordsValues_lengthT = *(inbuffer + offset++);
      if(loopWordsValues_lengthT > loopWordsValues_length)
        this->loopWordsValues = (rtabmap::KeyPoint*)realloc(this->loopWordsValues, loopWordsValues_lengthT * sizeof(rtabmap::KeyPoint));
      offset += 3;
      loopWordsValues_length = loopWordsValues_lengthT;
      for( uint8_t i = 0; i < loopWordsValues_length; i++){
      offset += this->st_loopWordsValues.deserialize(inbuffer + offset);
        memcpy( &(this->loopWordsValues[i]), &(this->st_loopWordsValues), sizeof(rtabmap::KeyPoint));
      }
     return offset;
    }

    const char * getType(){ return "rtabmap/InfoEx"; };
    const char * getMD5(){ return "9134640b305c7a8fe3e5322120ad0f22"; };
